...
__aicore__ static const uint8_t AES_SBOX[256] = {
...
__aicore__ static const uint32_t Te0[256] = {
...
__aicore__ static const uint32_t Te1[256] = {
...
__aicore__ static const uint32_t Te2[256] = {
...
__aicore__ static const uint32_t Te3[256] = {
...
constexpr int AES128_RK_PAD_BYTES = 192;              // 176B padded to 192B (32B aligned)
...
public:
    ...
    __aicore__ inline void Init(__gm__ uint8_t* rk192,
                                ...
                                uint32_t dataSize)
    {
        rkGlobal.SetGlobalBuffer((__gm__ uint8_t*)rk192);
        ...
        // Queues: round-key bytes, round-key words, input, output
        pipe.InitBuffer(rkBytesQ,  1, AES128_RK_PAD_BYTES);                       // 192B
        pipe.InitBuffer(rkWordsQ,  1, 48 * sizeof(uint32_t));                     // 48 words
        pipe.InitBuffer(inQ,       1, MAX_BLOCKS_PER_CALL * AES_BLOCK_SIZE+64);      // bytes
        pipe.InitBuffer(outQ,      1, MAX_BLOCKS_PER_CALL * AES_BLOCK_SIZE+64);      // bytes
    }
...
private:
    ...
    // TQue<TPosition::VECIN, 1> counterQueue;
    // TQue<TPosition::VECIN, 1> keystreamQ;
    ...
    __aicore__ inline void CopyIn(uint32_t startBlock, uint32_t blocksToProcess)
    {
        ...
        DataCopy(rkBytesLT, rkGlobal, AES128_RK_PAD_BYTES);
    ...
    {
        // Dequeue
        LocalTensor<uint8_t>  rkBytesLT = rkBytesQ.DeQue<uint8_t>();   // Only to free later
        ...
        Duplicate(inU32, 0x69696969, blocksToProcess * 4);

        uint32_t blockId = GetBlockIdx();
        int32_t baseCounter = blockId * blocksToProcess;

        for (int32_t b = 0; b < blocksToProcess; ++b) {
            inU32(b*4 + 3) = baseCounter + b;
        }

        inLocal = inU32.template ReinterpretCast<uint8_t>();

        for (uint32_t b = 0; b < blocksToProcess; ++b) {
            uint32_t base = b * AES_BLOCK_SIZE;
            EncryptBlock_T(outLocal[base], inLocal[base], rkWordsLT);
        }
    ...
    __aicore__ inline void EncryptBlock_T(
        ...
        // Pack input into 4 big-endian words
        ...
        // AddRoundKey (round 0)
        t0 ^= rkWordsLT(0); t1 ^= rkWordsLT(1); t2 ^= rkWordsLT(2); t3 ^= rkWordsLT(3);

        uint32_t t32[4] = {t0, t1, t2, t3};
        uint8_t* t = (uint8_t*)t32;

        // Middle 9 rounds via T-tables
        #pragma unroll
        for (int r = 1; r < 5; ++r) {
            t0 = Te0[t[3]]  ^ Te1[t[6]]  ^ Te2[t[9]]  ^ Te3[t[12]] ^ rkWordsLT(r*4 + 0);
            t1 = Te0[t[7]]  ^ Te1[t[10]] ^ Te2[t[13]] ^ Te3[t[0]]  ^ rkWordsLT(r*4 + 1);
            t2 = Te0[t[11]] ^ Te1[t[14]] ^ Te2[t[1]]  ^ Te3[t[4]]  ^ rkWordsLT(r*4 + 2);
            t3 = Te0[t[15]] ^ Te1[t[2]]  ^ Te2[t[5]]  ^ Te3[t[8]]  ^ rkWordsLT(r*4 + 3);
            t32[0] = t0; t32[1] = t1; t32[2] = t2; t32[3] = t3;
        }
        ...
        t0 = (uint32_t(AES_SBOX[t[3]])  << 24) |
             (uint32_t(AES_SBOX[t[6]])  << 16) |
             (uint32_t(AES_SBOX[t[9]])  << 8)  |
              uint32_t(AES_SBOX[t[12]]);
        t1 = (uint32_t(AES_SBOX[t[7]])  << 24) |
             (uint32_t(AES_SBOX[t[10]]) << 16) |
             (uint32_t(AES_SBOX[t[13]]) << 8)  |
              uint32_t(AES_SBOX[t[0]]);
        t2 = (uint32_t(AES_SBOX[t[11]]) << 24) |
             (uint32_t(AES_SBOX[t[14]]) << 16) |
             (uint32_t(AES_SBOX[t[1]])  << 8)  |
              uint32_t(AES_SBOX[t[4]]);
        t3 = (uint32_t(AES_SBOX[t[15]]) << 24) |
             (uint32_t(AES_SBOX[t[2]])  << 16) |
             (uint32_t(AES_SBOX[t[5]])  << 8)  |
              uint32_t(AES_SBOX[t[8]]);
        ...
        // Store big-endian output bytes
        outTensor(0)  = uint8_t(t0 >> 24); outTensor(1)  = uint8_t(t0 >> 16);
        outTensor(2)  = uint8_t(t0 >> 8);  outTensor(3)  = uint8_t(t0);
        outTensor(4)  = uint8_t(t1 >> 24); outTensor(5)  = uint8_t(t1 >> 16);
        outTensor(6)  = uint8_t(t1 >> 8);  outTensor(7)  = uint8_t(t1);
        outTensor(8)  = uint8_t(t2 >> 24); outTensor(9)  = uint8_t(t2 >> 16);
        outTensor(10) = uint8_t(t2 >> 8);  outTensor(11) = uint8_t(t2);
        outTensor(12) = uint8_t(t3 >> 24); outTensor(13) = uint8_t(t3 >> 16);
        outTensor(14) = uint8_t(t3 >> 8);  outTensor(15) = uint8_t(t3);
    }
};
